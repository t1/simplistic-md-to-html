...
fun main(args: Array<String>) {
    if (args.size != 1) throw IllegalArgumentException("Requires exactly one attributes: the name of the markdown file")
    val file = Paths.get(args[0])

    val html = SimplisticMarkDownToHtml(file).convert()

    Files.write(file.resolveSibling("README.html"), html.toByteArray())
}

class SimplisticMarkDownToHtml(private val text: String) {
    ...
    fun withBasePath(basePath: String): SimplisticMarkDownToHtml {
    ...
    fun map(pair: Pair<Pair<String, String>, String>): SimplisticMarkDownToHtml {
